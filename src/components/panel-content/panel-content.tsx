...
import {
  ...
  EVENTS,
  DYNAMIC_OVERLAYS_OPTIONS_STATE,
  PARAM_KEY,
} from "../../constants";
...
const PanelContent = () => {
  ...
  const [dynamicOverlaysOptions, setDynamicOverlaysOptions] = useAddonState<
    Record<string, DynamicOverlayOptions>
  >(DYNAMIC_OVERLAYS_OPTIONS_STATE, {});
  ...
  const updateOverlayOptions = useCallback(
    (options: DynamicOverlayOptions) => {
      setDynamicOverlaysOptions(
        Object.assign({}, dynamicOverlaysOptions, {
          [storyId]: {
            ...currentDynamicOverlayOptions,
            ...options,
          },
  ...
  const isDifferentFromDefault: (
    option: "all" | keyof DynamicOverlayOptions
  ) => boolean = useCallback(
    (option) => {
      if (option === "all") {
        return Object.keys(currentDynamicOverlayOptions).some(
          (optionName: keyof DynamicOverlayOptions) =>
            isDifferentFromDefault(optionName)
        );
      } else {
        if (!(`${option}` in currentDynamicOverlayOptions)) {
          ...
          return (
            currentDynamicOverlayOptions[option] !== parameter.overlay[option]
          );
        } else {
          return (
            currentDynamicOverlayOptions[option] !==
            DEFAULT_DYNAMIC_OVERLAY_OPTIONS[option]
          );
        }
      }
    },
  ...
  const resetOverlayOptions = useCallback(
    (option: "all" | keyof DynamicOverlayOptions) => {
        ...
        rows={[
          {
            ...
            control: (
              <Slider
                ...
                step={0.05}
                onChange={(_, value) =>
                  updateOverlayOptions({ opacity: value as number })
                }
                aria-label="Opacity"
          ...
          },
          {
            ...
            control: (
              <Checkbox
                ...
                onChange={(_, value) =>
                  updateOverlayOptions({ colorInversion: value })
                }
